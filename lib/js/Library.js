...
Zotero.Library.prototype.loadCollectionsSimple = function(config){
    ...
    var callback = J.proxy(function(data, textStatus, XMLHttpRequest){
        ...
        collectionsfeed.requestConfig = newConfig;
        //clear out display items
...
Zotero.Library.prototype.loadUpdatedItems = function(){
    ...
    itemVersionsDeferred.done(J.proxy(function(data, keysjqxhr){
        var updatedVersion = keysjqxhr.getResponseHeader("Zotero-Last-Modified-Version");
        library.items.syncState.earliestVersion = updatedVersion;
        library.items.syncState.latestVersion = updatedVersion;
        ...
        loadAllItemsDeferred.done(J.proxy(function(){
            if(library.items.syncState.earliestVersion == library.items.syncState.latestVersion){
                library.items.itemsVersion = library.items.syncState.latestVersion;
            }
...
Zotero.Library.prototype.loadUpdatedCollections = function(){
    ...
    collectionVersionsDeferred.done(J.proxy(function(data, keysjqxhr){
        var updatedVersion = keysjqxhr.getResponseHeader("Zotero-Last-Modified-Version");
        library.collections.syncState.earliestVersion = updatedVersion;
        library.collections.syncState.latestVersion = updatedVersion;
        ...
        loadAllCollectionsDeferred.done(J.proxy(function(){
            if(library.collections.syncState.earliestVersion == library.collections.syncState.latestVersion){
                library.collections.collectionsVersion = library.collections.syncState.latestVersion;
            }
            var displayParams = Zotero.nav.getUrlVars();
...
// getDeletedData
// process deleted
// checkConcurrentUpdates (compare Zotero-Last-Modified-Version from collections?newer request to one from /deleted request)
